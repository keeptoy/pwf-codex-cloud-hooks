...
- The local branch is named `0.3.0-beta.1` and its two commits use beta.1 wording while tracking `origin/0.3.0-alpha.2` and sitting two commits ahead. This is premature as release metadata because the frozen plan reserves beta.1 for Round 3 activation/acceptance. It is recorded as an experimental prototype label; no branch rename or history rewrite is authorized in this audit.
