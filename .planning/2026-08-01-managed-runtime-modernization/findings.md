...
- Recommended Round 3 policy defaults, pending maintainer confirmation: accept the trust claim as “regular inode reached through a contained no-symlink pathname” rather than attempt an unreliable hard-link-name proof; keep the tested portable `openat` walk as the required path and defer optional `openat2`; use a dedicated same-user 0700 temp base with exact-name/owner/mode/age-bounded stale cleanup plus `finally`; allocate the 30-second Host budget as owned-plan 8 seconds (resolver 2, injector 5, cleanup/result 1), owned catch-up 15 seconds, and 7 seconds adapter/process/JSON reserve.
