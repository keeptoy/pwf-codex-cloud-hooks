...
- Maintainer accepted documenting these four items as the Round 3 default freeze, conditional only on the remaining Cloud single-link compatibility gate. The prototype README now specifies exact single-link checkpoints/outcomes, a same-EUID `/tmp/pwf-codex-cloud-hooks-<euid>` base with 10-minute/32-entry/500-ms bounded cleanup, the portable `openat` production baseline, and the 27-second shared deadline split.
