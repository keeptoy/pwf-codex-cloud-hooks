...
def catchup(root: Path, skill: Path | None) -> str:
    ...
    # PWF_CODEX_CLOUD_COMPAT_PATCH_V1: the global Agent Skills path is under
    # ~/.agents, so the patched upstream runtime must not infer Claude from its
    # own location. The managed adapter knows that its host is Codex.
    environment["PWF_RUNTIME"] = "codex"
    # Setup-shell exports are not guaranteed to survive into a later Hook
    # process. The installed adapter lives at
    # $CODEX_HOME/hooks/planning-with-files/hook_adapter.py, so its owned path
    # is a stable fallback when Codex Cloud does not export CODEX_HOME.
    if not environment.get("CODEX_HOME", "").strip():
        environment["CODEX_HOME"] = str(Path(__file__).resolve().parents[2])
