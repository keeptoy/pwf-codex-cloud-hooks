...
> **Status:** version `0.2.1` is a deliberately small, read-only production
> baseline. It installs two managed Hook events. The larger upstream-runtime
> modernization is planned but is **not implemented yet**.
...
The current release registers exactly two managed events:

| Event | Matcher | Current action | Verification state |
|---|---|---|---|
| `SessionStart` | `startup\|resume\|clear\|compact` | Optionally runs upstream `session-catchup.py`, then injects the active plan and recent progress | `startup` and `resume` observed; forced `clear`/`compact` still pending |
...
Codex Cloud setup/maintenance
  ...
  v
install.js
  |-- verifies selected files in the global planning-with-files Skill
...
`upstream-manifest.json` also records the expected hashes of the selected Skill
files. `install.js` refuses installation when those files are missing or changed.
...
| `.planning/.active_plan` | Pointer to the modernization plan used by planning-with-files Hooks |
| `.planning/2026-08-01-managed-runtime-modernization/` | Durable roadmap, audit findings, decisions, and progress log |
...
bash -n init-cloud-sandbox-v0.2.1.bash
git diff --check
```

The Node suite currently contains nine **test cases**, not nine atomic product
...
`init-cloud-sandbox-v0.2.1.bash` is the pinned Debian/Ubuntu amd64 bootstrap. It
can install prerequisites, PowerShell, Node.js, the Skill, and the managed Hook
package, then validate the filesystem, TOML, Codex feature state, adapter
protocol, and canaries.

```bash
sudo bash init-cloud-sandbox-v0.2.1.bash all
bash init-cloud-sandbox-v0.2.1.bash help
bash init-cloud-sandbox-v0.2.1.bash verify
```
...
black-box verification printed by the script. Seeing a canary by manually
reading files is not proof that the lifecycle Hook ran; the canary must already
be present in the new session's runtime context.
...
- Phase 0, repository audit and roadmap capture, is complete.
- Phase 1, runtime provenance and compatibility contract, is next.
- Do not begin by enabling new Hook events.
- First define the upstream allowlist and dependency graph, manifest/provenance
  schema, deterministic import/check workflow, third-party notice, and golden
  compatibility fixtures.
- Phase 1 must leave installed Hook behavior unchanged.
...
3. Publish an immutable release archive.
4. Record the archive SHA-256 in the Cloud bootstrap.
5. Run bootstrap install and doctor in a fresh environment.
6. Start a new Cloud task and observe lifecycle canaries before manual file reads.
7. Keep canaries until every newly enabled lifecycle path is proven.
8. Remove canaries in a separately reviewed change and recompute production hashes.
