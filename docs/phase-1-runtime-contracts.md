...
> Contract version: 1
>
> Phase status: Phase 1 Round 1 complete locally; Rounds 2 and 3 pending
>
> Behavior status: contract-only; no runtime is imported, installed, or executed; current Hook behavior is unchanged

## Purpose

Round 1 freezes the trust boundary and supplies machine-readable example
requests/results. It answers five questions before any runtime import begins:
...
Entries marked `planned` are part of the frozen future ZIP boundary but do not
exist yet. Round 2 must create and verify them before changing their state to
`present`; Round 3 must prove the expanded lifecycle and compatibility goldens.

## Round boundaries

- Round 1: freeze these contracts and tests; no Hook behavior change.
- Round 2: implement deterministic import/check, overlay application, manifest
  expansion, and MIT attribution; do not change Hook behavior.
- Round 3: freeze full Cloud/golden fixtures, extend installer lifecycle tests,
  prove compatibility, and build the alpha.1 candidate.
